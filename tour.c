...
int send_packet(char sourcevm[6],char dest[6],char packet[MAXLINE])
{
	...
	for (ifa = ifaddr, n = 0; ifa != NULL; ifa = ifa->ifa_next, n++) {
		...
		if (ifa->ifa_addr->sa_family == AF_INET) {
			...
			if(strncmp(host, "127", 3) == 0 || strncmp(host, "192", 3) == 0)
				continue;
			else {
				//	printf("\t\taddress: %s\n", host);
				strcpy(srcIP, host);
	...
	addr_list = (struct in_addr **)he2->h_addr_list;
	for(int i = 0; he2->h_addr_list[i] != NULL; i++) {
		inet_ntop(AF_INET, he2->h_addr_list[i], srcIP, INET_ADDRSTRLEN);
		printf("src-%s\n", srcIP);
		/* if((srcIP[0] == '1' && srcIP[1] == '2' && srcIP[2] == '7') ||
		   (srcIP[0] == '1' && srcIP[1] == '9' && srcIP[2] == '2'))
		//if(strncmp(srcIP, "127", 3) == 0 || strncmp(srcIP, "192", 3) == 0)
		continue;*/
	}
	printf("Source IP : %s \n",srcIP);
	...
	/*addr_list = (struct in_addr **)he3->h_addr_list;
	  for(int i = 0; addr_list[i] != NULL; i++) {
	  inet_ntop(AF_INET, addr_list, destIP, INET_ADDRSTRLEN);
	  if(strncmp(destIP, "127", 3) == 0 || strncmp(destIP, "192", 3) == 0)
	  continue;
	  }
	...
	printf("\nSending packet on route traversal socket to next node in tour %s\n\n", dest);
...
int areq (struct sockaddr *IPaddr, socklen_t sockaddrlen, struct hwaddr *HWaddr)
{
	...
	if (connect(unixdomain_socket, (struct sockaddr*)&arp_address, sizeof(arp_address)) < 0)
		printf("\n Error in connecting to unix socket in areq. Check if areq is running %d\n", errno);

	if(nbytes_send = write(unixdomain_socket, IPaddress, INET_ADDRSTRLEN)<0)
		printf(" Error in writing to the connection socket.Check if areq is running %d\n", errno);
...
int multicastreply(){
	...
	sprintf(multicastmsg, "Multicast<<<<<Node %s .I am a member of the group. >>>>> \n", currentnode);
	tourendflag = 1;
...
int main(int argc, char const *argv[])
{
	...
	while(1)
	{
		...
		if (FD_ISSET(rt, &rset)) {
			...
			rt_data = Recvfrom(rt, rtbuffer , sizeof rtbuffer, 0, (struct sockaddr *)&rtaddr, &sock_len);
			if(rt_data < 0) {
			...
			if(ntohs(rt_recv_hdr->id) == IDENTIFICATION) {
				...
				if(rt_recv_payload[1] == (recvlen-6)) {
					...
					sprintf(multicastmsg, "Multicast<<<<< This is node %s .  Tour has ended .  Group members please identify yourselves. >>>>> \n", currentnode);
					sendmulticastmsg(multicastmsg);
					//continue;
			...
			continue;
		} else if (FD_ISSET(udprecv_socket, &rset)) { 	//if packet is received on unix domain socket
