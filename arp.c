...
#define PROTOCOL 6171      //Unique protocol
...
int printEthArpFrame(struct Ethernet_hdr* printEthHdr ,arp_hdr* printARPhdr )
{
	...
	printf("\n Identification - %u",ntohs(printARPhdr->target_mac));

	printf("\n");  
	printf("ARP MESSAGE-----COMPLETE---- \n");
...
int ip_hwaddr()
{
	...
	for (hwahead = hwa = Get_hw_addrs(); hwa != NULL; hwa = hwa->hwa_next) {
		char *IP;
		if ( (sa = hwa->ip_addr) != NULL)
			IP = Sock_ntop_host(sa, sizeof(*sa));
		//if (strncmp(hwa->if_name, "ens",3) == 0) {
		if (strncmp(IP, "127",3) != 0 && strncmp(IP, "192", 3) != 0 ) {
			printf("%s :%s", hwa->if_name, ((hwa->ip_alias) == IP_ALIAS) ? " (alias)\n" : "\n");
...
int process_arp_reply()
{
	if(strcmp(ip_canonical,parphdr_rcv->target_ip)==0) {
		...
		printf("\n ARP reply received at the source ARP module");
		printEthArpFrame(pethframehdr_rcv,parphdr_rcv);
		send_arp_unix();
...
int main(int argc, char *argv[])
{
	...
	while(1) {
		...
		if (FD_ISSET(unixdomain_socket, &rset)) {
			...
			*/
			get_local_IP(sourceCanonicalIP);
			printf("\n source Canonical IP : %s \n", sourceCanonicalIP);
